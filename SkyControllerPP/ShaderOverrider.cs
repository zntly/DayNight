	...
	public class ShaderOverrider
	{
		...
		public static void Postfix(GlobalShaderColors __instance)
		{
			if (Utils.IsBTOS2())
			{
				...
				if (SkyInfo.Phase == "Night" && ModSettings.GetString("Jail Cell Shading Mode") == "Night" && skyColor != JailCellShader.lastcolor)
				{
			...
			else
			{
				...
				if (SkyInfo.Phase == "Night" && ModSettings.GetString("Jail Cell Shading Mode") == "Night" && skyColor2 != JailCellShader.lastcolor)
				{
					JailCellShader.ShadeCell(skyColor2);
				}
				return;
			}
		}
